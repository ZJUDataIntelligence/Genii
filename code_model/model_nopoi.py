...
class Spatial_Net1(torch.nn.Module):
    def __init__(self, num_node_features1, num_node_features2):
        ...
        self.gat1 = GATv2Conv(num_node_features1, out_channels=1, heads=20, dropout=0.4)
        self.han = HAN(-1, out_channels=20)
        self.re = nn.ReLU()
        self.fcn = nn.Linear(34, 7)
...
class Model(torch.nn.Module):
    ...
    def forward(self, s_data, t_data):
        ...
        x_s = self.s_fcn(x).reshape(1, -1)
        x_t = self.t_fcn(x_t).reshape(1, -1)
        # x_t = x_t.unsqueeze(1)
