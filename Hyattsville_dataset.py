...
class Dataset_Net1(InMemoryDataset):
    ...
    def processed_file_names(self):
        return ['Hyattsville_litter_graph.pt']
        # filename_list = []
        # for i in range(840):
    ...
    def process(self):
        ...
        for i in range(116):
            ...
            for j in range(1, 5):
                ...
                # POI_litter
                # 路径
                poi_litter_edge_path = './Hyattsville_poi_litter_edge/poi_litter_day{0}_cluster{1}.csv'.format(i, j)
                litter_path = './Hyattsville_litter_days_df/day{0}_litter{1}.csv'.format(i, j)
                litter_edge_path = './Hyattsville_litter_days_edge/day{0}_litter{1}.csv'.format(i, j)
                litter_ad_path = './Hyattsville_litter_days_df/litter_ad.csv'
                poi_path = 'Hyattsville_POI_all/poi.csv'
...
b = Dataset_Net1("Hyattsville_graph")
print(b.data[0])

# data_loader = DataLoader(b, batch_size=1, shuffle=False)  # 加载数据进行处理，每批次数据的数量为1
# for i, data in enumerate(data_loader, 1):
#     print(data)
b1 = torch.load('./Hyattsville_graph/processed/Hyattsville_litter_graph.pt')
