...
def train(day_train):
    ...
    return np.mean(train_loss), pred, val_loss


@torch.no_grad()
def val(day_train):
    model.eval()
    lable = y[day_train:145]
    pred = []
    loss = {}
    with torch.no_grad():
        for i in range(day_train, 145):
            out = model(dataset[i], tw_train[i])
            out = out.cpu().reshape(1).detach().numpy()
            if out < 0:
                out = 0
...
if __name__ == '__main__':
    ...
    for epoch in range(200):
        train_loss, train_pred, val_loss = train(108)
        ...
        if val_loss['val_rmse_loss'] < min_rmse_loss and val_loss['val_mae_loss'] < min_mae_loss:
            ...
            # if epoch % 10 == 0:
            torch.save(model, 'Krommenie_model/model_K_gcn_0045e4.pth')
    ...
    loss,pred = test(145, 'Krommenie_model/model_K_gcn_0045e4.pth')
    print(loss)

    # plt.plot(pred, label="pred")
    # plt.plot(y[145:], label="test")
    # # plt.plot(train_pred, label="train_pred")
    # # plt.plot(y[:126], label="train")
    # plt.legend()
    # plt.savefig('./Krommenie_model/model_K_allGCN_0055e5.png')
    # plt.show()
