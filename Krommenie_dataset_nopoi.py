...
class Dataset_Net1(InMemoryDataset):
    ...
    def processed_file_names(self):
        return ['Krommenie_litter_graph.pt']
        # filename_list = []
        # for i in range(840):
    ...
    def process(self):
        ...
        for i in range(181):
            ...
            for j in range(1, 5):
                ...
                litter_path = './Krommenie_litter_days_df/day{0}_litter{1}.csv'.format(i, j)
                litter_edge_path = './Krommenie_litter_days_edge/day{0}_litter{1}.csv'.format(i, j)
                ...
                # POI_litter
                # 路径
                poi_litter_edge_path = './Krommenie_poi_litter_edge/poi_litter_day{0}_cluster{1}.csv'.format(i, j)
                litter_path = './Krommenie_litter_days_df/day{0}_litter{1}.csv'.format(i, j)
                litter_edge_path = './Krommenie_litter_days_edge/day{0}_litter{1}.csv'.format(i, j)
                litter_ad_path = './Krommenie_litter_days_df/litter_ad.csv'
                poi_path = 'Krommenie_POI_all/poi.csv'
                ...
                # 放入datalist
                data_day.append([dataset_litter, data_poi_litter])
...
b = Dataset_Net1("Krommenie_graph_nopoi")
print(b.data[0])

# data_loader = DataLoader(b, batch_size=1, shuffle=False)  # 加载数据进行处理，每批次数据的数量为1
# for i, data in enumerate(data_loader, 1):
#     print(data)
b1 = torch.load('./Krommenie_graph_nopoi/processed/Krommenie_litter_graph.pt')
print(b1[0][0][1].edge_index_dict['litter', 'distance', 'poi'].size())
print(b1[0][1][1].edge_index_dict['litter', 'distance', 'poi'].size())
print(b1[0][2][1].edge_index_dict['litter', 'distance', 'poi'].size())
print(b1[0][3][1].edge_index_dict['litter', 'distance', 'poi'].size())
print(b1[1][0][1].edge_index_dict['litter', 'distance', 'poi'].size())
print(b1[1][0][1].metadata())
